...
- 光标锚定缩放、适应窗口/宽度/高度、100% 原尺寸模式、双击切换适配与 100%；高倍率像素网格；切换图像保持当前缩放比。
- 可选局部放大镜（Ctrl+M）：窗口随鼠标位置移动，8 倍显示光标附近像素，带中心十字与当前像素框。
- 当前目录自然排序、异步缩略图、图像两侧翻图箭头、左右方向键；F5 刷新目录。缩略图固定为小尺寸，左上角显示序号/总数；支持 Ctrl+T 显隐、滚轮横向浏览和当前项自动定位，界面不显示滚动条。
- 深色圆角窗口，菜单栏与标题栏融合为单行，默认 13px 界面字体，跟随 Windows 高 DPI 缩放；状态栏实时显示零起点坐标与 RGBA 值。
- 图像区右键菜单：打开所在文件夹、复制图像文件、全屏显示（F11 进入，Esc 退出）、删除（Delete，默认移入回收站并自动切到相邻图像）。
...
- 多窗口与多实例：文件菜单“新建窗口”(Ctrl+N) 再开独立窗口查看其他文件夹；设置中开启“允许多实例”后，从系统打开图片每次都会启动独立窗口，不再转发到已有窗口。
