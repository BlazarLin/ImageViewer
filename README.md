...
![RGB 三通道分析界面](docs/images/260908统一分析面板.png)
...
- ROI 通道最小值、最大值、均值、标准差；彩色图在同一张图中叠加 R/G/B 三通道直方图（共用纵轴，颜色及线型区分），灰度图显示单通道。图表包含刻度，可悬停查看每档计数。
...
3. 鼠标移到图像上查看状态栏像素值。`Ctrl+I` 打开分析面板，`Shift + 左键拖动` 选择 ROI；像素、直方图和 ROI 统计在同一面板内显示，也可点击“分析整张图”。
...
回归程序返回非零即失败，当前 37 项检查覆盖解码、导航失败恢复、拖放事件、处理状态、ROI、RGB 直方图、缓存失效、异步请求、坐标取样、两侧箭头、方向键焦点隔离、关于信息、灰度色相及圆角状态。本轮使用体验、改进建议和待回复项见 [深度体验记录](docs/260908深度体验与优化建议.md)。UI 自动回归使用 offscreen；实际桌面高 DPI、拖放和多显示器仍需人工验收。
